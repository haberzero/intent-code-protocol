...
- `src_ibc/`: 行为描述层源码目录。存放行为描述层文件，后缀名为 `.ibc`。缩写含义：model context behavior
- `src_target/` 或 `src_tmain/`: 目标源代码目录。存放最终生成或手动编写的目标语言源代码文件（如 `.c`, `.cpp`, `.py`, `.java` 等）

**重要原则：** 

`src_ibc/`, `src_target/` 目录下的子目录结构必须完全相同，且位于相同路径下的文件应具有相同的文件名，仅文件后缀不同

除此之外，符号表文件也会同时分布在src_ibc的各个子文件夹下，每一层级的文件夹内的icp_symbols将会各自独立管理其当前路径内文件的对应符号表
...
- `src_ibc/utils/math_c.ibc` 
- `src_ibc/utils/math_h.ibc`

(C语言的文件后缀处理)

如果 `src_target/scripts/process.py` 存在且兼容性配置要求额外后缀，则对应：

- `src_ibc/scripts/process_py.ibc`
如果 `src_target/scripts/process.py` 存在且兼容性配置不要求额外后缀，则对应：

- `src_ibc/scripts/process.ibc`

## 4. 文件命名约定

文件的命名必须遵循以下约定，特别是对于 `.ibc` 文件，需要根据对应的目标源代码文件类型进行特殊处理，以确保唯一性和清晰度

a.  **基本后缀名**
    - 行为描述层文件后缀名：固定为 `.ibc`。

b.  **针对C/C++语言系列的特殊处理**
    如果目标源代码文件属于C语言系列（常见的后缀有 `.c`, `.cpp`, `.h`, `.hpp` 等），其对应的行为描述层 (`.ibc`) 文件名，应在原文件名末尾、扩展名（`.ibc`）之前，增加一个特定的额外后缀。这个额外后缀根据原C/C++文件的类型决定：
    - `.h`, `.hpp` (头文件) -> 额外后缀 `_h`
    - `.c` (C源文件) -> 额外后缀 `_c`
    - `.cpp`, `.cc`, `.cxx` (C++源文件) -> 额外后缀 `_cpp` `_cc` `_cxx`

例如:
    - 目标文件 `main.h` -> 对应ibc文件: `main_h.ibc`
    - 目标文件 `utils.c` -> 对应ibc文件: `utils_c.ibc`
    - 目标文件 `geometry.cpp` -> 对应ibc文件: `geometry_cpp.ibc`

    在工程配置文件（参见第5项）中，对于C语言系列（如语言类型标记为 `"c"`, `"cpp"` 等），其 `is_extra_suffix` 属性应**固定为 `true`**，且其额外后缀的生成规则硬编码为上述逻辑

c.  **针对其他语言系列的处理**
    对于非C/C++的其他目标语言（如 Python, Java, JavaScript 等），其对应的行为描述层 (`.ibc`) 文件名是否添加额外后缀，由工程配置配置文件中的 `is_extra_suffix` 布尔属性控制

    - 如果在工程配置文件中，对应语言的 `is_extra_suffix` 属性为 `true`：
        在应在原文件名末尾、扩展名（`.ibc`）之前，增加一个额外后缀
        示例:
        - 目标文件 `script.py`, `is_extra_suffix: true` -> 对应ICP文件: `script_py.ibc`
        - 目标文件 `App.java`, `is_extra_suffix: true` -> 对应ICP文件: `App_java.ibc`

    - 如果在工程配置文件中，对应语言的 `is_extra_suffix` 属性为 `false` 或未设置：
        无额外后缀
        示例:
        - 目标文件 `script.py`, `is_extra_suffix: false` -> 对应ICP文件: `script.ibc`

**文件命名示例总结表:**

| 目标文件 (示例) | 目标文件类型 / 扩展名 | 兼容性配置 `is_extra_suffix` | `.ibc` 文件 (示例) | 规则依据 |
| :-------------- | :-------------------- | :--------------------------- | :---------------------- | :---------------------- | :------- |
| `header.h`      | C/C++ Header (`.h`)   | `true` (固定)                | `header_h.ibc`          | C/C++ 规则 (`_h`) |
| `source.c`      | C Source (`.c`)       | `true` (固定)                | `source_c.ibc`          | C/C++ 规则 (`_c`) |
| `module.cpp`    | C++ Source (`.cpp`)   | `true` (固定)                | `module_cpp.ibc`        | C/C++ 规则 (`_cpp`) |
| `util.py`       | Python (`.py`)        | `true`                       | `util_py.ibc`           | 其他语言规则 (`_py`) |
| `util.py`       | Python (`.py`)        | `false` 或 未设置             | `util.ibc`              | 其他语言规则 (无额外后缀) |
