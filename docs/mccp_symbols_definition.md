...
`mccp_symbols.json` 文件在 MCCP 项目中扮演着**核心注册表**的角色。它集中记录了项目中跨越不同层级（行为描述层 `.mcbh`、符号-伪代码层 `.mcpc` 和目标代码层 `src_target`）的所有关键**符号**（Symbol）信息。
...
1.  **手动维护**: 开发者在添加、修改或删除符号时，手动编辑 `mccp_symbols.json` 文件。这种方式适用于小型项目或早期原型，但容易出错且工作量大。
...
推荐结合半自动和全自动的方式，例如主要通过扫描 `.mcpc` 文件自动生成，但允许开发者手动添加额外信息（如 `description`）或覆盖自动提取的细节。

保持 `mccp_symbols.json` 的准确和最新，是确保 MCCP 工具链高效、可靠运行的关键。
