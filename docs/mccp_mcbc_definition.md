...
意图注释位于特定的关键字“下方”或某个代码块的“上方”，且单独占据一行

当意图注释位于某个“没有子代码块”的关键字下方时，意图注释仅作用于这个关键字上

比如，位于`input`关键字下方的意图注释就仅修饰输入参数表，位于`func`关键字下方的意图注释也不修饰行为代码，而是修饰函数定义本身

当意图注释位于某个“有子代码块”的关键字下方、同时也就位于一个子代码块的上方时，意图注释会作用于这个关键字以及这个代码块内的所有代码

比如，位于`behavior`关键字下方的意图注释，就会作用于这个代码块内的所有代码行。又比如当意图注释位于一个`if-else`关键字下方时，意图注释作用于这个`if-else`代码块内的所有代码，不作用于其外的代码。

`class`关键字被认为是一个“有子代码块”的关键字，因此`class`下方的意图注释会作用于这个`class`代码块内的所有代码行，不作用于`class`代码块外的代码行。
...
3. 意图注释不被允许在行为描述代码段中零碎使用，必须针对完整代码块
...
- 如果存在意图注释，`description` 关键字应该出现在意图注释之后。`description`本身不允许被意图注释修饰，这毫无意义。
...
虽然MCCP-toolchain不会进行强制限制，但`module`关键字只应该出现在每个文件的最上方。在行为代码过程中改变module调用容易导致意想不到的错误

`module`关键字随后跟随的是一个文件名，这个文件名可以仅有文件名，也可以跟随着具体的路径


## 8. 缩进约定 (indentation convention)
