...
[MCCP-Toolchain](https://github.com/haberzero/mccp-toolchain) 构建中。当 MCCP-Toolchain 的第一个demo版本完善、说明书编写完成后，本仓库版本号将会来到v0.0.3。
...
- v0.0.3: 初步实现 MCCP 的正向构建功能，目前计划将MCCP-ToolChain放置在新repo（同时尝试将MCCP工作流嵌入到类似Trae的AI-IDE的工作流程中去）。
...
- 模型兼容性： MCCP 在设计之初就始终在思考如何让相对小尺寸的模型也能介入对大型工程的高效开发，从而允许企业能以相对低成本的方式合理地引入并部署MCCP 技术。
...
- 针对MCCP-Toolchain（规划中）设计并开放兼容性接口/结构，允许不同开发者能够灵活根据所使用的模型自定义MCCP工程的行为，但同时保持对标准MCCP工程的兼容性。
- 通过MCCP的工程结构，完成对MCCP-Toolchain的自举。
